"""개인 파생 질문 생성 Chain (P2) - Langchain LCEL 사용"""
...
class PersonalGenerateChain:
    """개인 파생 질문 생성 Chain (RAG 기반) - Langchain LCEL"""
    ...
    async def generate(
        ...
        try:
            logger.info(
                f"[개인 질문 생성 시작] member_id={member_id}, "
                f"role={role_label}, rag_count={len(rag_results)}"
            )
            ...
            # 3. LCEL Chain 실행 (Langsmith 자동 트레이싱)
            ...
            logger.info(
                f"[개인 질문 생성 완료] question='{result['question'][:30]}...', "
    ...
    def _format_rag_context(self, rag_results: list[dict[str, Any]]) -> str:
        ...
        formatted_lines = []
        for idx, result in enumerate(rag_results[:5], 1):  # 최대 5개
            line = (
